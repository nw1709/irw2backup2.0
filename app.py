...
def call_anthropic(prompt, images_base64):
    ...
    try:
        response = anthropic_client.messages.create(model=CLAUDE_MODEL_NAME, max_tokens=5000, messages=[{"role": "user", "content": content}], timeout=90.0)
...
def call_gpt_o3(system_prompt, user_prompt, images_base64):
    ...
    try:
        response = openai_client.chat.completions.create(model=GPT_MODEL_NAME, messages=messages, max_completion_tokens=5000, timeout=90.0)
